...
//    items      ..addAll(_points);
  ...
  Positioned _tooltip(
    ...
    if (side == 'left') {
      prefix = _tooltipData.leftPrefix ?? const SizedBox();
      suffix = _tooltipData.leftSuffix ?? const SizedBox();
      if (widget.rangeSlider == false) {
        return const Positioned(child: SizedBox());
      }
    } else {
      prefix = _tooltipData.rightPrefix ?? const SizedBox();
      suffix = _tooltipData.rightSuffix ?? const SizedBox();
    }
...
//            alignment: Alignment.center,
  ...
  Positioned _centralWidget() {
    return Positioned(
      ...
      right: 0,
      bottom: 0,
      child: Center(child: widget.trackBar.centralWidget ?? const SizedBox()),
    );
  }
